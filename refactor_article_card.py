...
with open('c:/Users/roezm/IMPORTANT/fp/front_and_ruang_it/lib/app/widgets/article_card.dart', 'w', encoding='utf-8') as f:
    f.write(card_code)

def replace_in_file(filepath, has_delete=True):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # 1. Add import
    if "import '../../../widgets/article_card.dart';" not in content:
        content = content.replace("import 'package:flutter/material.dart';", "import 'package:flutter/material.dart';\nimport '../../../widgets/article_card.dart';")
        
    # 2. Find and replace `_buildArticleCard(context, article)` inside `build` methods
    if has_delete:
        content = re.sub(
            r'_buildArticleCard\(\s*context\s*,\s*article\s*\)',
            r'ArticleCard(article: article, onLikeToggle: (id) => controller.toggleLike(id), onDelete: (id) => controller.deleteArticle(id))',
            content
        )
    else:
        content = re.sub(
            r'_buildArticleCard\(\s*context\s*,\s*article\s*\)',
            r'ArticleCard(article: article, onLikeToggle: (id) => controller.toggleLike(id))',
            content
        )
        
    # 3. Remove `_buildArticleCard`, `_buildSnippetPreview`, `_showArticleOptions`, `_showDeleteConfirmation` from the view class
    # Because these methods are long and at the end, we can truncate the file before `_buildArticleCard` assuming it's the last few methods.
    # Actually, we can use regex to remove them, but since we know they start with `Widget _buildArticleCard`, we can slice the string.
    
    match = re.search(r'\s+Widget _buildArticleCard\(BuildContext context, dynamic article\) \{', content)
    if match:
...
# Dashboard has delete
replace_in_file('c:/Users/roezm/IMPORTANT/fp/front_and_ruang_it/lib/app/modules/dashboard/views/dashboard_view.dart', True)
# Profile has delete
replace_in_file('c:/Users/roezm/IMPORTANT/fp/front_and_ruang_it/lib/app/modules/profile/views/profile_view.dart', True)
# Explore has no delete
replace_in_file('c:/Users/roezm/IMPORTANT/fp/front_and_ruang_it/lib/app/modules/explore/views/explore_view.dart', False)
# Search has no delete
replace_in_file('c:/Users/roezm/IMPORTANT/fp/front_and_ruang_it/lib/app/modules/search/views/search_view.dart', False)
# Author Profile has no delete
replace_in_file('c:/Users/roezm/IMPORTANT/fp/front_and_ruang_it/lib/app/modules/author_profile/views/author_profile_view.dart', False)
